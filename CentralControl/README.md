...
1. 検索ウィンドウに`Start Naming Service`と入力し，`ネーミングサービス`を起動する．起動後はコマンドプロンプトのウィンドウが表示されるが，ここで何かを操作する必要は無い．
...
### コンポーネントの配置
作成したコンポーネントを配置する．
1. 検索ウィンドウに`power shell`と入力し，`PowerShell`を起動する．
2. `python`と入力し，実行するPythonファイルをPowerShellのウィンドウ内にドラッグまたはPythonファイルのパスを入力する．(`python`の後ろは半角スペースを入れる必要がある．)
3. `Enter`キーを押してプログラムを実行する．この時点では，Pythonプログラムの`onInitialize`の中身が実行される．
4. プログラムが実行されると，Eclispeの方にコンポーネントが表示される．
5. `ON`をクリックし，システムエディタを有効にする．
6. Pythonファイルをシステムエディタのウィンドウ内にドラッグしてコンポーネントを配置する．配置後はドラッグで移動できる．
7. 必要なプログラムを全て実行し，コンポーネントを配置する．ノートPC上で実行するプログラムはノートPC上で2.，3.を実施する．ノートPCで実行したプログラムのコンポーネントは，研PCのEclipseに表示される．
![コンポ－ネント配置](https://github.com/user-attachments/assets/73fa9838-72c5-492d-8ddd-e22b7a9504c9)




 ### コンポーネントの接続
 配置したコンポーネントを接続する．コンポーネントの凸型の図形部分(出力ポート)から，接続したいコンポーネントの凹型の図形部分(入力ポート)までドラッグする．このとき接続できるのは，同じデータタイプのものである．プログラム上で定義したデータポートのタイプが異なると接続できないため注意する．
 ![コンポーネント接続](https://github.com/user-attachments/assets/bd49bdda-fec3-4c0f-930b-0fb96297a31c)
